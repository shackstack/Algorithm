function solution(fees, records) {
  ...
  records.forEach((value) => {
    ...
    if (type === 'IN') {
      if (amountMap.has(carNumber)) {
        amountMap.set(carNumber, {
          ...amountMap.get(carNumber),
    ...
    if (type === 'OUT') {
      ...
      amountMap.set(carNumber, {
        interval: amountMap.get(carNumber).interval + interval,
        entryTime: null,
      });
    }
  });
  ...
  const result = amountArr.sort().map(([key, value]) => {
    ...
    if (entryTime) {
      ...
      const minuteInterval = endTime.minute - entryMinute;
      const hourInterval = endTime.hour - entryHour;
